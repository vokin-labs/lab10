...
void Creator::create_new_random_db(const std::string& path_to_new_db) {
  ...
  for(size_t i = 0; i < column_families_names.size(); ++i){
    for(size_t j = 0; j < static_cast<size_t>(num_in_each_column[i]); ++j, ++count){
      status = db_ptr->Put(write_options, handles[i], "key" + std::to_string(count+1), values[count]);
      if(!status.ok())
  ...
  for (int i = 0; i < max_len_col; ++i){
    ...
    for(size_t j = 0; j < column_families_names.size(); ++j){
      if(num_in_each_column[j] > i){
        status = db_ptr->Get(rocksdb::ReadOptions(), handles[j], "key" + std::to_string(count_in_line+1), &temp);
        if(!status.ok())
